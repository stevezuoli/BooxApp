...
void ThumbnailView::setCurrentPage(const QString bookmark)
{
    ...
    emit needThumbnailForNewPage(bookmark, size);
}
...
void ThumbnailView::setThumbnail(ThumbPtr thumb)
{
    ...
    else
    {
        ...
        emit needNextThumbnail(thumb->path(), size);
    }
}
...
void ThumbnailView::readyToShow()
{
    ...
    // update page bar
    emit positionChanged(display_pages_.front()->key(), static_cast<int>(model_->documentClient()->getPageCount()));
    onyx::screen::instance().setDefaultWaveform(onyx::screen::ScreenProxy::GU);
    update();
}
...
void ThumbnailView::nextScreen()
{
    ...
    emit needNextThumbnail(last_page->path(), size);
}
...
void ThumbnailView::prevScreen()
{
    ...
    emit needPreviousThumbnail(first_page->path(), size);
}
...
void ThumbnailView::setPreviousThumbnail(ThumbPtr thumb)
{
    ...
    else
    {
        ...
        emit needPreviousThumbnail(thumb->path(), size);
    }
}
...
void ThumbnailView::paintSketches(QPainter &painter, ThumbPtr page, int index)
{
    ...
    sketch_proxy_->setZoom( page->zoom() );
...
void ThumbnailView::resizeEvent(QResizeEvent *re)
{
    ...
    if (display_pages_.size() > 0)
    {
        setCurrentPage(display_pages_.get_page(0)->path());
    }
}
    ...
    case Qt::LeftButton:
        {
                ...
                thumb_idx < static_cast<int>(display_pages_.size()))
            {
                ...
                emit returnToReading(display_pages_.get_page(thumb_idx)->path());
            }
        }
        break;
    case Qt::RightButton:
...
void ThumbnailView::keyReleaseEvent(QKeyEvent *ke)
{
    ...
    case Qt::Key_Enter:
        {
            sys::SysStatus::instance().setSystemBusy(true, false);
            emit returnToReading(display_pages_.get_page(cur_thumb_index_)->path());
        }
        break;
    case Qt::Key_Escape:
        {
...
{
    if (value >= 0 && value <= model_->documentClient()->getPageCount())
    {
        AdobeLocationPtr loc = model_->documentClient()->getLocationFromPagePosition(value);
        setCurrentPage(loc->getBookmark());
    }
}
