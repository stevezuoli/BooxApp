...
public:
	CHMTextStream(CHMFileInfo &chmFile, fb::shared_ptr<ZLInputStream> base);

private:
	void resetToStart();
	fb::shared_ptr<ZLInputStream> nextStream();
...
CHMTextStream::CHMTextStream(CHMFileInfo &chmFile, fb::shared_ptr<ZLInputStream> base) : myCHMFile(chmFile), myBase(base) {
}
...
void CHMTextStream::resetToStart() {
	...
	fb::shared_ptr<ZLInputStream> tocStream = myCHMFile.entryStream(myBase, names.TOC);
...
fb::shared_ptr<ZLInputStream> CHMTextStream::nextStream() {
	while (myIndex < myEntryNames.size()) {
		fb::shared_ptr<ZLInputStream> stream = myCHMFile.entryStream(myBase, myEntryNames[myIndex++]);
		if (!stream.isNull()) {
...
bool CHMPlugin::readDescription(const std::string &path, DBBook &book) const {
	...
	/*
	fb::shared_ptr<ZLInputStream> entryStream = chmFile.entryStream(stream, names.Start);
...
bool CHMPlugin::readModel(const DBBook &book, BookModel &model) const {
	...
	fb::shared_ptr<CHMFileInfo> info = new CHMFileInfo(book.fileName());
	if (!info->init(*stream)) {
		return false;
	}
	...
	fb::shared_ptr<ZLInputStream> tocStream = info->entryStream(stream, names.TOC);
	...
	*/
	...
	while (referenceCollection.containsNonProcessedReferences()) {
		...
		if (ZLStringUtil::stringEndsWith(fileName, ".jpg") ||
			...
			fb::shared_ptr<ZLInputStream> entryStream = info->entryStream(stream, fileName);
