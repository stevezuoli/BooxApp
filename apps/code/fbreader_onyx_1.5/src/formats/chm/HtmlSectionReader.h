...
public:
	HtmlSectionReader(BookModel &model, const PlainTextFormat &format, const std::string &encoding, fb::shared_ptr<CHMFileInfo> info, CHMReferenceCollection &collection);
...
private:
	fb::shared_ptr<HtmlTagAction> createAction(const std::string &tag);

private:
	fb::shared_ptr<CHMFileInfo> myInfo;
