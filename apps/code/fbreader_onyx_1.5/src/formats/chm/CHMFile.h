...
public:
	...
	fb::shared_ptr<ZLInputStream> entryStream(fb::shared_ptr<ZLInputStream> base, const std::string &name) const;
	// We assume that base exists and is already open
	FileNames sectionNames(fb::shared_ptr<ZLInputStream> base) const;
...
public:
	CHMInputStream(fb::shared_ptr<ZLInputStream> base, const CHMFileInfo::SectionInfo &sectionInfo, size_t offset, size_t size);
	~CHMInputStream();
...
private:
	...
	fb::shared_ptr<LZXDecompressor> myDecompressor;
	std::string myInData;
