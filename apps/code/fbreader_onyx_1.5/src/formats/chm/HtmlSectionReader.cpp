...
fb::shared_ptr<HtmlTagAction> HtmlSectionReader::createAction(const std::string &tag) {
	if (tag == "IMG") {
...
HtmlSectionReader::HtmlSectionReader(BookModel &model, const PlainTextFormat &format, const std::string &encoding, fb::shared_ptr<CHMFileInfo> info, CHMReferenceCollection &collection) : HtmlBookReader("", model, format, encoding), myInfo(info), myReferenceCollection(collection) {
