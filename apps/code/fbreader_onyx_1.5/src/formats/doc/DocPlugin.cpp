...
bool DocPlugin::readDescription(const std::string &path, DBBook &book) const {
    ZLFile file(path);
    /*
...
bool DocPlugin::readModel(const DBBook &book, BookModel &model) const {
    return DocBookReader(model, book.encoding()).readDocument(book.fileName());
}
