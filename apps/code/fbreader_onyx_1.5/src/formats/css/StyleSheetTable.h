...
public:
	...
	static fb::shared_ptr<ZLTextStyleEntry> createControl(const AttributeMap &map);
...
public:
	...
	fb::shared_ptr<ZLTextStyleEntry> control(const std::string &tag, const std::string &aClass) const;
...
private:
	...
	std::map<Key,fb::shared_ptr<ZLTextStyleEntry> > myControlMap;
