...
std::string OEBPlugin::opfFileName(const std::string &oebFileName) {
	...
	fb::shared_ptr<ZLDir> zipDir = oebFile.directory(false);
	if (zipDir.isNull()) {
		return "";
	}
...
public:
	InputStreamLock(fb::shared_ptr<ZLInputStream> stream);

private:
	fb::shared_ptr<ZLInputStream> myStream;
};

InputStreamLock::InputStreamLock(fb::shared_ptr<ZLInputStream> stream) : myStream(stream) {
}
