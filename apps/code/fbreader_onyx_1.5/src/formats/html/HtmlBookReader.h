...
protected:
	virtual fb::shared_ptr<HtmlTagAction> createAction(const std::string &tag);
...
private:
	...
	fb::shared_ptr<StyleSheetParser> myStyleSheetParser;

	int mySpaceCounter;
	int myBreakCounter;
	std::string myConverterBuffer;

	std::map<std::string,fb::shared_ptr<HtmlTagAction> > myActionMap;
