...
fb::shared_ptr<HtmlTagAction> HtmlBookReader::createAction(const std::string &tag) {
	if (tag == "EM") {
...
bool HtmlBookReader::tagHandler(const HtmlTag &tag) {
	...
	fb::shared_ptr<HtmlTagAction> action = myActionMap[tag.Name];
	if (action.isNull()) {
...
void HtmlBookReader::startDocumentHandler() {
	...
	myDontBreakParagraph = false;
	for (std::map<std::string,fb::shared_ptr<HtmlTagAction> >::const_iterator it = myActionMap.begin(); it != myActionMap.end(); ++it) {
		it->second->reset();
	}
	myIsStarted = false;
