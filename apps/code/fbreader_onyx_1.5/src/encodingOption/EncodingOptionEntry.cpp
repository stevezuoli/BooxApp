...
AbstractEncodingEntry::AbstractEncodingEntry(const std::string &currentValue) {
	...
	const std::vector<fb::shared_ptr<ZLEncodingSet> > &sets = ZLEncodingCollection::instance().sets();
	for (std::vector<fb::shared_ptr<ZLEncodingSet> >::const_iterator it = sets.begin(); it != sets.end(); ++it) {
