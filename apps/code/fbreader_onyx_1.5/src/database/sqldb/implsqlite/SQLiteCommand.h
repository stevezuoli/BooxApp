...
public:
	...
	bool execute();
	fb::shared_ptr<DBValue> executeScalar();
	fb::shared_ptr<DBDataReader> executeReader();
...
private:
	...
	struct BindParameter {
		...
		BindParameter(unsigned pos) : Position(pos), Name("") {}
		BindParameter(unsigned pos, const std::string &name) : Position(pos), Name(name) {}
	...
	bool bindParameters();
	bool bindParameterByName(const std::string &name, fb::shared_ptr<DBValue> value);
	bool bindParameterByIndex(unsigned index, fb::shared_ptr<DBValue> value);
	bool bindParameter(sqlite3_stmt *statement, int number, fb::shared_ptr<DBValue> value);
