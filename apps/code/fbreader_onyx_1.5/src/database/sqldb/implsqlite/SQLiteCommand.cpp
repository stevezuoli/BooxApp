...
std::string SQLiteCommand::packCommand(const std::string &command) {
	...
	while (true) {
		...
		stripped[pos++] = ' ';
		next = stripped.find_first_not_of(_spaces, pos);
...
fb::shared_ptr<DBValue> SQLiteCommand::executeScalar() {
	SQLiteConnection &con = (SQLiteConnection &) connection();
	if (!con.isOpened()) {
		myStatements.clear();
		return 0;
	...
	while (true) {
		...
		case SQLITE_OK:
			break;
		case SQLITE_ROW: {
				fb::shared_ptr<DBValue> val = SQLiteDataReader::makeDBValue(*it, /* column = */ 0);
				resetStatements();
				return val;
			}
		default:
...
fb::shared_ptr<DBDataReader> SQLiteCommand::executeReader() {
	SQLiteConnection &con = (SQLiteConnection &) connection();
	if (!con.isOpened()) {
		myStatements.clear();
		return 0;
...
bool SQLiteCommand::bindParameterByName(const std::string &name, fb::shared_ptr<DBValue> value) {
	const unsigned size = myStatements.size();
	bool res = true;
	bool binded = false;
...
bool SQLiteCommand::bindParameterByIndex(unsigned index, fb::shared_ptr<DBValue> value) {
	if (index == 0) {
		return true;
	}
	const unsigned size = myStatements.size();
...
bool SQLiteCommand::bindParameter(sqlite3_stmt *statement, int number, fb::shared_ptr<DBValue> value) {
