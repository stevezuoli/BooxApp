...
bool SaveAuthorsRunnable::run() {
	...
	const std::vector<fb::shared_ptr<DBAuthor> > &bookAuthors = myBook->authors(); // save link to vector
	...
	unsigned index = 0;
	for (std::vector<fb::shared_ptr<DBAuthor> >::const_iterator it = bookAuthors.begin(); it != bookAuthors.end(); ++it) {
