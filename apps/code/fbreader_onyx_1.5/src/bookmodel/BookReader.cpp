...
void BookReader::setFootnoteTextModel(const std::string &id) {
	std::map<std::string,fb::shared_ptr<ZLTextModel> >::iterator it = myModel.myFootnotes.find(id);
	if (it != myModel.myFootnotes.end()) {
		myCurrentTextModel = (*it).second;
	} else {
		myCurrentTextModel = new ZLTextPlainModel(8192);
          myModel.myFootnotes.insert(std::pair<std::string,fb::shared_ptr<ZLTextModel> >(id, myCurrentTextModel));
	}
}
...
void BookReader::addImage(const std::string &id, fb::shared_ptr<const ZLImage> image) {
