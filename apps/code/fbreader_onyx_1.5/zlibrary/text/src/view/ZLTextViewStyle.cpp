...
ZLTextView::ViewStyle::ViewStyle(fb::shared_ptr<ZLPaintContext> context) : myContext(context), myBaseBidiLevel(0) {
	setTextStyle(ZLTextStyleCollection::instance().baseStylePtr(), myBaseBidiLevel);
	myWordHeight = -1;
}

void ZLTextView::ViewStyle::setPaintContext(fb::shared_ptr<ZLPaintContext> context) {
	myContext = context;
}
