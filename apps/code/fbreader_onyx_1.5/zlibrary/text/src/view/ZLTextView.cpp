...
ZLTextView::ZLTextView(ZLApplication &application, fb::shared_ptr<ZLPaintContext> context) : ZLView(application, context), myPaintState(NOTHING_TO_PAINT), myOldWidth(-1), myOldHeight(-1), myStyle(context), mySelectionModel(*this, application), myTreeStateIsFrozen(false), myDoUpdateScrollbar(false) {
}
...
void ZLTextView::setPaintContext(fb::shared_ptr<ZLPaintContext> context) {
...
void ZLTextView::setModel(fb::shared_ptr<ZLTextModel> model, const std::string &language) {
	clear();
...
void ZLTextView::copySelectedTextToClipboard(ZLDialogManager::ClipboardType type) const {
	if (ZLDialogManager::instance().isClipboardSupported(type)) {
		...
		if (!text.empty()) {
			...
			fb::shared_ptr<ZLImageData> image = mySelectionModel.image();
			if (!image.isNull()) {
