...
private:
	...
	public:
		ViewStyle(fb::shared_ptr<ZLPaintContext> context);
		~ViewStyle();
		void setPaintContext(fb::shared_ptr<ZLPaintContext> context);
	...
	private:
		...
		fb::shared_ptr<ZLPaintContext> myContext;
...
protected:
	ZLTextView(ZLApplication &application, fb::shared_ptr<ZLPaintContext> context = 0);
	virtual ~ZLTextView();
	void setPaintContext(fb::shared_ptr<ZLPaintContext> context);
	virtual fb::shared_ptr<ZLTextPositionIndicatorInfo> indicatorInfo() const = 0;
...
public:
	...
	virtual void setModel(fb::shared_ptr<ZLTextModel> model, const std::string &language);
	const fb::shared_ptr<ZLTextModel> model() const;
...
protected:
	...
	virtual fb::shared_ptr<PositionIndicator> createPositionIndicator(const ZLTextPositionIndicatorInfo&);
...
private:
	...
	fb::shared_ptr<ZLTextView::PositionIndicator> positionIndicator();
...
private:
	fb::shared_ptr<ZLTextModel> myModel;
	...
	fb::shared_ptr<PositionIndicator> myPositionIndicator;
...
inline const fb::shared_ptr<ZLTextModel> ZLTextView::model() const { return myModel; }
