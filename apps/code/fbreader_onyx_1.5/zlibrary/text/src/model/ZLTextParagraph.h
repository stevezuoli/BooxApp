...
class ZLImage;
typedef std::map<std::string, fb::shared_ptr<const ZLImage> > ZLImageMap;
...
public:
	...
	~ZLTextControlEntryPool();
	fb::shared_ptr<ZLTextParagraphEntry> controlEntry(ZLTextKind kind, bool isStart);

private:
	std::map<ZLTextKind, fb::shared_ptr<ZLTextParagraphEntry> > myStartEntries;
	std::map<ZLTextKind, fb::shared_ptr<ZLTextParagraphEntry> > myEndEntries;
};
...
public:
	...
	fb::shared_ptr<const ZLImage> image() const;
	short vOffset() const;
...
public:
	static const fb::shared_ptr<ZLTextParagraphEntry> Instance;
...
public:
	...
	public:
		...
		bool isEnd() const;
		void next();
		const fb::shared_ptr<ZLTextParagraphEntry> entry() const;
	...
	private:
		...
		size_t myIndex;
		size_t myEndIndex;
		mutable fb::shared_ptr<ZLTextParagraphEntry> myEntry;
	};
