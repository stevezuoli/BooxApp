...
ZLQtOptionsDialog::ZLQtOptionsDialog(const ZLResource &resource, fb::shared_ptr<ZLRunnable> applyAction, bool showApplyButton) : QDialog(qApp->activeWindow()), ZLDesktopOptionsDialog(resource, applyAction) {
	setModal(true);
...
void ZLQtOptionsDialog::selectTab(const ZLResourceKey &key) {
	for (std::vector<fb::shared_ptr<ZLDialogContent> >::const_iterator it = myTabs.begin(); it != myTabs.end(); ++it) {
		if ((*it)->key() == key.Name) {
...
bool ZLQtOptionsDialog::runInternal() {
	for (std::vector<fb::shared_ptr<ZLDialogContent> >::iterator it = myTabs.begin(); it != myTabs.end(); ++it) {
