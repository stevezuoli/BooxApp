...
void ZLQtApplicationWindow::addToolbarItem(ZLToolbar::ItemPtr item) {
	...
	switch (item->type()) {
		...
		case ZLToolbar::Item::MENU_BUTTON:
		{
			...
			fb::shared_ptr<ZLPopupData> popupData = buttonItem.popupData();
...
ZLQtRunPopupAction::ZLQtRunPopupAction(QObject *parent, fb::shared_ptr<ZLPopupData> data, size_t index) : QAction(parent), myData(data), myIndex(index) {
...
void ZLQtApplicationWindow::setToolbarItemState(ZLToolbar::ItemPtr item, bool visible, bool enabled) {
	...
	switch (item->type()) {
		...
		{
			ZLToolbar::MenuButtonItem &buttonItem = (ZLToolbar::MenuButtonItem&)*item;
			fb::shared_ptr<ZLPopupData> data = buttonItem.popupData();
