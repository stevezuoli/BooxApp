...
fb::shared_ptr<ZLMessageOutputChannel> ZLWin32CommunicationManager::createMessageOutputChannel(const std::string &protocol, const std::string &testFile) {
	if (protocol != "execute") {
		return 0;
	}
...
fb::shared_ptr<ZLMessageSender> ZLWin32ExecMessageOutputChannel::createSender(const ZLCommunicationManager::Data &data) {
