...
fb::shared_ptr<ZLDialog> ZLWin32DialogManager::createDialog(const ZLResourceKey &key) const {
	return new ZLWin32Dialog(*myApplicationWindow, resource()[key]);
}

fb::shared_ptr<ZLOptionsDialog> ZLWin32DialogManager::createOptionsDialog(const ZLResourceKey &key, fb::shared_ptr<ZLRunnable> applyAction, bool showApplyButton) const {
