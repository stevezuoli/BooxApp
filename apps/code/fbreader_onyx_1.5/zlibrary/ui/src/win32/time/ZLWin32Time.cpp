...
void ZLWin32TimeManager::execute(UINT taskId) {
	fb::shared_ptr<ZLRunnable> task = myTasks[taskId];
	if (task) {
		task->run();
	}
}

void ZLWin32TimeManager::addTask(fb::shared_ptr<ZLRunnable> task, int interval) {
...
void ZLWin32TimeManager::removeTask(fb::shared_ptr<ZLRunnable> task) {
	if (ZLWin32ApplicationWindow::ourApplicationWindow != 0) {
		HWND window = ZLWin32ApplicationWindow::ourApplicationWindow->myMainWindow;
		if (window != 0) {
			UINT id = myIds[task];
