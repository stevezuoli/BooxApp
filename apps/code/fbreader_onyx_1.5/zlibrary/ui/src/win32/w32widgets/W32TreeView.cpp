...
void W32TreeView::insert(const std::string &itemName, HICON icon) {
	...
	fb::shared_ptr<W32TreeViewItem> item = new W32TreeViewItem(itemName, iconIndex);
...
void W32TreeView::init(HWND parent, W32ControlCollection *collection) {
	...
	for (std::vector<fb::shared_ptr<W32TreeViewItem> >::iterator it = myItems.begin(); it != myItems.end(); ++it) {
