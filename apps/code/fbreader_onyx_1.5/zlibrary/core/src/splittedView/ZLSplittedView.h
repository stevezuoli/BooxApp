...
public:
	ZLSplittedView(ZLApplication &application, fb::shared_ptr<ZLPaintContext> context = 0);
	~ZLSplittedView();

	void paint();

	void setLeftView(fb::shared_ptr<ZLView> view);
	void setRightView(fb::shared_ptr<ZLView> view);
...
private:
	fb::shared_ptr<ZLView> myLeftView;
	fb::shared_ptr<ZLView> myRightView;
	bool myMoveSplitter;
};
