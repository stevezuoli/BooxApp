...
void XMLConfig::load() {
	...
	fb::shared_ptr<ZLDir> configDir = ZLFile(configDirName()).directory(false);
	if (!configDir.isNull()) {
...
void XMLConfig::saveAll() {
	...
	fb::shared_ptr<ZLDir> configDir = ZLFile(configDirName()).directory(true);
	...
	if (myDelta != 0) {
		if (!configDir.isNull()) {
			...
			for (std::set<std::string>::const_iterator it = categories.begin(); it != categories.end(); ++it) {
				if (!it->empty()) {
					fb::shared_ptr<ZLOutputStream> stream = ZLFile(configDir->itemPath(*it + ".xml")).outputStream();
...
void XMLConfig::saveDelta()
{
    ...
    if (!configDir.isNull())
    {
        fb::shared_ptr<ZLOutputStream> stream =
