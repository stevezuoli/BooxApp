...
public:
	virtual fb::shared_ptr<ZLMessageOutputChannel> createMessageOutputChannel(const std::string &protocol, const std::string &testFile) = 0;
	virtual void addInputMessageDescription(const std::string &command, const std::string &protocol, const Data &data) = 0;
	void registerHandler(const std::string &command, fb::shared_ptr<ZLMessageHandler> receiver);

private:
	fb::shared_ptr<ZLMessageHandler> handler(const std::string &command);

private:
    std::map<std::string, fb::weak_ptr<ZLMessageHandler> >	myRegisteredHandlers;
	bool myInitialized;
...
public:
	...
	virtual fb::shared_ptr<ZLMessageSender> createSender(const ZLCommunicationManager::Data &data) = 0;
};
