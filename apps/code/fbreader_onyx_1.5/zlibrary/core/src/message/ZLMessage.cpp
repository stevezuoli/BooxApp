...
public:
	fb::shared_ptr<ZLMessageOutputChannel> createMessageOutputChannel(const std::string&, const std::string&) { return 0; }
...
void ZLCommunicationManager::registerHandler(const std::string &command, fb::shared_ptr<ZLMessageHandler> receiver) {
	myRegisteredHandlers[command] = receiver;
}

fb::shared_ptr<ZLMessageHandler> ZLCommunicationManager::handler(const std::string &command) {
    std::map<std::string, fb::weak_ptr<ZLMessageHandler> >::iterator it = myRegisteredHandlers.find(command);
...
void ZLCommunicationManager::onMessageReceived(const std::string &command, const std::vector<std::string> &arguments) {
	fb::shared_ptr<ZLMessageHandler> h = handler(command);
	if (!h.isNull()) {
