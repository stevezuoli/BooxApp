...
std::map<std::string, fb::weak_ptr<ZLInputStream> > ZLFile::ourPlainStreamCache;
...
fb::shared_ptr<ZLInputStream> ZLFile::envelopeCompressedStream(fb::shared_ptr<ZLInputStream> &base) const {
	if (base != 0) {
...
fb::shared_ptr<ZLInputStream> ZLFile::inputStream() const {
	fb::shared_ptr<ZLInputStream> stream;
	...
	if (index == -1) {
		...
		fb::shared_ptr<ZLInputStream> base = baseFile.inputStream();
		if (!base.isNull()) {
...
fb::shared_ptr<ZLOutputStream> ZLFile::outputStream() const {
	if (isCompressed()) {
		return 0;
	}
...
fb::shared_ptr<ZLDir> ZLFile::directory(bool createUnexisting) const {
	if (exists()) {
		if (isDirectory()) {
...
void ZLFile::fillInfo() const {
	...
	if (index == -1) {
		...
		if (archive.exists()) {
			fb::shared_ptr<ZLDir> dir = archive.directory();
			if (!dir.isNull()) {
