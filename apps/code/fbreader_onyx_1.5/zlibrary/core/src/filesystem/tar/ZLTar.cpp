...
const ZLTarHeaderCache &ZLTarHeaderCache::cache(ZLInputStream &baseStream) {
	...
	fb::shared_ptr<ZLUserData> data = baseStream.getUserData(tarHeaderMapKey);
	if (data.isNull()) {
...
ZLTarInputStream::ZLTarInputStream(fb::shared_ptr<ZLInputStream> &base, const std::string &name) : myBaseStream(new ZLInputStreamDecorator(base)), myCompressedFileName(name) {
}
