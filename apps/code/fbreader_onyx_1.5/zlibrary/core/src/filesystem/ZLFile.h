...
private:
	static std::map<std::string, fb::weak_ptr<ZLInputStream> > ourPlainStreamCache;
...
public:
	...
	fb::shared_ptr<ZLInputStream> inputStream() const;
	fb::shared_ptr<ZLOutputStream> outputStream() const;
	fb::shared_ptr<ZLDir> directory(bool createUnexisting = false) const;

private:
	void fillInfo() const;
	fb::shared_ptr<ZLInputStream> envelopeCompressedStream(fb::shared_ptr<ZLInputStream> &base) const;
