...
bool ZLEncodingConverterInfo::canCreateConverter() const {
	...
	const std::vector<fb::shared_ptr<ZLEncodingConverterProvider> > &providers = collection.providers();
	for (std::vector<fb::shared_ptr<ZLEncodingConverterProvider> >::const_iterator it = providers.begin(); it != providers.end(); ++it) {
...
fb::shared_ptr<ZLEncodingConverter> ZLEncodingConverterInfo::createConverter() const {
	ZLEncodingCollection &collection = ZLEncodingCollection::instance();
	const std::vector<fb::shared_ptr<ZLEncodingConverterProvider> > &providers = collection.providers();
	for (std::vector<fb::shared_ptr<ZLEncodingConverterProvider> >::const_iterator it = providers.begin(); it != providers.end(); ++it) {
		for (std::vector<std::string>::const_iterator jt = myAliases.begin(); jt != myAliases.end(); ++jt) {
