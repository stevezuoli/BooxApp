...
public:
	...
	fb::shared_ptr<ZLEncodingConverter> createConverter() const;
...
typedef fb::shared_ptr<ZLEncodingConverterInfo> ZLEncodingConverterInfoPtr;
...
public:
	const std::vector<fb::shared_ptr<ZLEncodingSet> > &sets();
	ZLEncodingConverterInfoPtr info(const std::string &name);
	ZLEncodingConverterInfoPtr info(int code);
	fb::shared_ptr<ZLEncodingConverter> defaultConverter();
	void registerProvider(fb::shared_ptr<ZLEncodingConverterProvider> provider);

private:
	void addInfo(ZLEncodingConverterInfoPtr info);
	const std::vector<fb::shared_ptr<ZLEncodingConverterProvider> > &providers() const;

private:
	std::vector<fb::shared_ptr<ZLEncodingSet> > mySets;
	std::map<std::string,ZLEncodingConverterInfoPtr> myInfosByName;
	std::vector<fb::shared_ptr<ZLEncodingConverterProvider> > myProviders;
