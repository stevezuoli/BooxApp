...
ZLApplication::ZLApplication(const std::string &name)
        : ZLApplicationBase(name),
          ...
          DocumentPathOption(ZLCategoryKey::CONFIG, CONFIG, "DocumentPath", ""),
...
ZLApplication::~ZLApplication() {
    QString path = QString::fromLocal8Bit(DocumentPathOption.value().c_str());
    saveDocumentOptions(path);
    saveBookmarks(path);
...
bool ZLApplication::initWindow() {
    loadDocumentOptions(QString::fromLocal8Bit(DocumentPathOption.value().c_str()));
...
void ZLApplication::addAction(const std::string &actionId, fb::shared_ptr<Action> action) {
    myActionMap[actionId] = action;
}

void ZLApplication::setView(fb::shared_ptr<ZLView> view) {
	if (view.isNull()) {
        return;
    }
...
fb::shared_ptr<ZLView> ZLApplication::currentView() const {
...
fb::shared_ptr<ZLApplication::Action> ZLApplication::action(const std::string &actionId) const {
    std::map<std::string,fb::shared_ptr<Action> >::const_iterator it = myActionMap.find(actionId);
	return (it != myActionMap.end()) ? it->second : 0;
}

bool ZLApplication::isActionVisible(const std::string &actionId) const {
    fb::shared_ptr<Action> a = action(actionId);
	return !a.isNull() && a->isVisible();
}

bool ZLApplication::isActionEnabled(const std::string &actionId) const {
    fb::shared_ptr<Action> _action = action(actionId);
...
Bookmarks & ZLApplication::bookmarks()
{
    if (!bookmarks_loaded_)
    {
        ...
        vbf::loadBookmarks(database, path, bookmarks_);
...
void ZLApplication::doActionByKey(const std::string &key) {
    fb::shared_ptr<ZLKeyBindings> bindings = keyBindings();
	if (bindings.isNull()) {
        return;
    }
    fb::shared_ptr<Action> a = action(bindings->getBinding(key));
	if (!a.isNull() &&
        (!a->useKeyDelay() ||
...
fb::shared_ptr<ZLPaintContext> ZLApplication::context() {
    return myContext;
}
...
void ZLApplicationWindow::setVisualParameter(const std::string &id, const std::string &value) {
    std::map<std::string,fb::shared_ptr<VisualParameter> >::iterator it = myParameterMap.find(id);
...
const std::string &ZLApplicationWindow::visualParameter(const std::string &id) {
    std::map<std::string,fb::shared_ptr<VisualParameter> >::const_iterator it = myParameterMap.find(id);
    if (it != myParameterMap.end()) {
...
void ZLApplicationWindow::addVisualParameter(const std::string &id, fb::shared_ptr<VisualParameter> parameter) {
	if (!parameter.isNull()) {
...
fb::shared_ptr<ZLKeyBindings> ZLApplication::keyBindings() {
	return 0;
}
