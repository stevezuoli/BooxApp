...
void ZLApplication::registerPopupData(const std::string &actionId, fb::shared_ptr<ZLPopupData> popupData) {
	toolbar(ZLApplicationWindow::WINDOW_TOOLBAR);
	myToolbar->registerPopupData(actionId, popupData);
}

void ZLToolbar::registerPopupData(const std::string &actionId, fb::shared_ptr<ZLPopupData> popupData) {
...
fb::shared_ptr<ZLPopupData> ZLToolbar::MenuButtonItem::popupData() const {
	std::map<std::string,fb::shared_ptr<ZLPopupData> >::const_iterator it = toolbar().myPopupDataMap.find(actionId());
...
ZLToolbar::ButtonGroup &ZLToolbar::getButtonGroup(const std::string &id) {
	fb::shared_ptr<ButtonGroup> group = myButtonGroups[id];
	if (group.isNull()) {
