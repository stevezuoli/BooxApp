...
void ZLImageManager::convertMultiImage(const ZLMultiImage &multiImage, ZLImageData &data) const {
	...
	if ((rows > 0) && (columns > 0)) {
		std::vector<fb::shared_ptr<ZLImageData> > parts;
		...
		for (unsigned int i = 0; i < rows; ++i) {
			for (unsigned int j = 0; j < columns; ++j) {
				fb::shared_ptr<const ZLImage> subImage = multiImage.subImage(i, j);
				if (subImage.isNull()) {
					return;
				}
				fb::shared_ptr<ZLImageData> data = imageData(*subImage);
				int w = data->width();
				if (i == 0) {
					widths.push_back(w);
					fullWidth += w;
...
fb::shared_ptr<ZLImageData> ZLImageManager::imageData(const ZLImage &image) const {
	fb::shared_ptr<ZLImageData> data = createData();

	if (image.isSingle()) {
		const ZLSingleImage &singleImage = (const ZLSingleImage&)image;
		fb::shared_ptr<std::string> stringData = singleImage.stringData();
