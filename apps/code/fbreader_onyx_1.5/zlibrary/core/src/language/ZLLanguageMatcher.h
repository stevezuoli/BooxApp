...
public:
	ZLLanguageMatcher(fb::shared_ptr<ZLLanguageDetector::LanguageInfo> info);
	virtual ~ZLLanguageMatcher();

	fb::shared_ptr<ZLLanguageDetector::LanguageInfo> info() const;

private:
	fb::shared_ptr<ZLLanguageDetector::LanguageInfo> myInfo;
};

class ZLStatisticsBasedMatcher : public ZLLanguageMatcher {

public:
	ZLStatisticsBasedMatcher(const std::string &fileName, fb::shared_ptr<ZLLanguageDetector::LanguageInfo> info);
	~ZLStatisticsBasedMatcher(); // надо ли его объявлять, если он ничего не делает??

	int criterion(const ZLStatistics &otherStatistics);

private:
	fb::shared_ptr<ZLArrayBasedStatistics> myStatisticsPtr;
};
