...
fb::shared_ptr<ZLStatisticsItem> ZLMapBasedStatistics::begin() const {
	return new ZLMapBasedStatisticsItem(myDictionary.begin(), 0);
}

fb::shared_ptr<ZLStatisticsItem> ZLMapBasedStatistics::end() const {
...
void ZLArrayBasedStatistics::insert(const ZLCharSequence &charSequence, size_t frequency) {
	...
	for(size_t i = 0; i < myCharSequenceSize; ++i) {
        std::string str = charSequence.toString();
        if (str.length() > i)
        {
            mySequences[myBack * myCharSequenceSize + i] = str[i];
        }
	}
...
fb::shared_ptr<ZLStatisticsItem> ZLArrayBasedStatistics::begin() const {
	return new ZLArrayBasedStatisticsItem(mySequences, myFrequencies, 0);
}

fb::shared_ptr<ZLStatisticsItem> ZLArrayBasedStatistics::end() const {
