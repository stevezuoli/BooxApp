...
public:
	static fb::shared_ptr<ZLTreeResource> ourRoot;
...
private:
	bool myHasValue;
	std::string myValue;
	std::map<std::string,fb::shared_ptr<ZLTreeResource> > myChildren;
...
public:
	ZLResourceTreeReader(fb::shared_ptr<ZLTreeResource> root);
...
private:
	std::stack<fb::shared_ptr<ZLTreeResource> > myStack;
};
...
private:
	static fb::shared_ptr<ZLMissingResource> ourInstance;
...
fb::shared_ptr<ZLTreeResource> ZLTreeResource::ourRoot;
fb::shared_ptr<ZLMissingResource> ZLMissingResource::ourInstance;
...
const ZLResource &ZLTreeResource::operator [] (const std::string &key) const {
	std::map<std::string,fb::shared_ptr<ZLTreeResource> >::const_iterator it = myChildren.find(key);
...
ZLResourceTreeReader::ZLResourceTreeReader(fb::shared_ptr<ZLTreeResource> root) {
	myStack.push(root);
}
...
void ZLResourceTreeReader::startElementHandler(const char *tag, const char **attributes) {
	if (!myStack.empty() && (NODE == tag)) {
		...
		if (name != 0) {
			...
			fb::shared_ptr<ZLTreeResource> node = myStack.top()->myChildren[sName];
			if (node.isNull()) {
				if (value != 0) {
