...
protected:
	static void addPlatformDependentBuilder(fb::shared_ptr<ZLDialogContentBuilder> builder);

private:
	static std::vector<fb::shared_ptr<ZLDialogContentBuilder> > ourPlatformDependentBuilders;

protected:
	ZLOptionsDialog(const ZLResource &resource, fb::shared_ptr<ZLRunnable> applyAction);
...
protected:
	fb::shared_ptr<ZLRunnable> myApplyAction;
	std::vector<fb::shared_ptr<ZLDialogContent> > myTabs;
};
