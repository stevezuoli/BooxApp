...
, current_speed_(2)
{
    ...
    // Identify current speaker according to package files.
    current_speaker_.clear();
    foreach(QString s, en_speakers)
    {
        if (packageExists("en", s))
        {
            current_speaker_ = s;
            break;
        }
    ...
    if (current_speaker_.isEmpty())
    {
        foreach(QString s, cn_speakers)
        {
            if (packageExists("cn", s))
            {
                current_speaker_ = s;
                break;
            }
...
bool EJSound::package(const QString &language, const QString & speaker, std::string & name)
{
    ...
    qDebug("package %s", qPrintable(package_));
...
// This function checks package exists or not for given language and speaker.
bool EJSound::packageExists(const QString & language,
                            const QString & speaker)
{
    QString p = prefix() + packageName(language, speaker);
    qDebug("Checking package %s", qPrintable(p));
...
Q_EXPORT_PLUGIN2(tts_ej, EJSound);
