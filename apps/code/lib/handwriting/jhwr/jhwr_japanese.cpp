...
static QString getDicPathByLocal(const QLocale & locale)
{
    ...
    dir.cd("handwriting");
    {
        QString path = dir.filePath("japanese.dat");
...
bool jHWRJapanese::initialize(const QLocale & locale)
{
    ...
    {
        HZSetParam(PARAM_RECORANG, RECOG_RANGE_GESTURE | RECOG_RANGE_0208 | RECOG_RANGE_JIS |
                   RECOG_RANGE_KATAKANA | RECOG_RANGE_HIRAGANA | RECOG_RANGE_ADAPTATION);
        return true;
    }
#endif
    return false;
...
bool jHWRJapanese::deinitialize()
{
    if (address_ == 0)
    {
        return false;
...
bool jHWRJapanese::setWorkArea(const QRect & rect)
{
    return false;
}

bool jHWRJapanese::setCandidateNum(const int number)
{
    if (address_ == 0)
    {
        return false;
...
bool jHWRJapanese::setLocale(const QLocale & locale)
{
    if (address_ == 0)
    {
        return false;
...
bool jHWRJapanese::setSpecialRecognizeRange(SpecialRecognizeRange range)
{
#ifdef BUILD_FOR_ARM
    switch (range)
    {
...
bool jHWRJapanese::recognize(QStringList & result)
{
    if (address_ == 0)
    {
        return false;
...
bool jHWRJapanese::recognizeSingleChar(HZ_INT16 * points, QStringList & result)
{
    result.clear();
    HZ_WORD * code_array = new HZ_WORD[candidates_num_ + 20];
    code_array[candidates_num_] = 0;
