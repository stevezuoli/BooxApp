...
static int setup_fd(int fd)
{
    ...
    write(fd, "1", 1);
    fprintf(stderr, "Enable wacom now\n\n\n");
    return(1);
}
...
#ifdef DEBUG
    // fprintf(stderr, "%dx%d flag: %d\tcontact: %d\teraser: %d\tbutton: %d\n",
    //       samp->x, samp->y, samp->flags, buttons & 0x01, buttons & 0x04, buttons & 0x02);
#endif
    return 1;
}
...
#ifdef DEBUG
    fprintf(stderr, "Initializing IDSV4 module\n");
#endif
