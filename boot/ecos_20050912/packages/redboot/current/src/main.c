...
static void eink_init(int want_update)
{
    int i = 0;
    void *err_addr;
    ...
    unsigned char* tmp = malloc(SCREEN_WIDTH * SCREEN_HEIGHT);
    flash_read((void *)0x100000, tmp, SCREEN_WIDTH * SCREEN_HEIGHT, (void **)&err_addr);

    if (want_update)
    {
        // Show "Software update request detected, checking updates..."
        memset(tmp + (SCREEN_HEIGHT - UPDIMG_HEIGHT - 4) * SCREEN_WIDTH, 0xFF, (UPDIMG_HEIGHT + 4) * SCREEN_WIDTH);
        int start_x = 2;
        int start_y = SCREEN_HEIGHT - 2 - UPDIMG_HEIGHT;

        for (i=0; i<UPDIMG_HEIGHT; i++)
        {
            memcpy(tmp + SCREEN_WIDTH * (start_y + i) + start_x, upd_image[i], UPDIMG_WIDTH);
        }
    }
...
cyg_start(void)
{
    ...
    mxc_kpp_init();
    for (res = 0; res < 1024; res++)
    {
        if (mxc_kpp_scan_matrix() == 7)
        {
            want_update = 1;
            break;
        }
    ...
    eink_init(want_update);

    if (want_update)
    {
        strcpy(script, "fis load initramfs;exec -b 0x100000 -l 0x300000 -c \"noinitrd "
            "console=ttymxc0,115200 root=/dev/mtdblock3 rootfstype=yaffs2 init=/linuxrc\"");
    }
    else
    {
        strcpy(script, "fis load kernel;exec -b 0x100000 -l 0x200000 -c \"noinitrd "
            "console=ttymxc0,115200 root=/dev/mtdblock3 rootfstype=yaffs2 init=/linuxrc\"");
    }
