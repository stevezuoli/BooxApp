...
static struct nand_oobinfo oob_layout =
{
    ...
    .oobfree = { {0, 5}, {11, 10}, {27, 10}, {43, 10}, {59, 5} }
};
...
{
    int i = 0, j = 0;
    unsigned char* src_buf = (unsigned char*)pt;

    for (i = 0; i < sizeof(yaffs_PackedTags2);)
    {
        if (oob_layout.oobfree[j][1] == 0)
        {
            fprintf(stderr, "No OOB space for tags");
            exit(-1);
        }

        memcpy(buf+oob_layout.oobfree[j][0], src_buf+i, oob_layout.oobfree[j][1]);
        i += oob_layout.oobfree[j][1];
        j++;
    }
}
